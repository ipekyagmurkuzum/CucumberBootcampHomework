...
public class VotesSteps {
    ...
    @When("I will create one more vote for this {string} with this {string}")
    public void iWillCreateOneMoreVote(String sub_id, String image_id) {
        String requestBody = "{\n" +
                "  \"image_id\": \"" + image_id + "\",\n" +
                "  \"sub_id\": \"" + sub_id + "\"" + ",\n" +
                "  \"value\": \"add\"\n}";
    ...
    @Then("I have numbers plus one votes for this {string} and check whether {string} is correct")
    public void iHaveNumbersPlusOneVotes(String sub_id, String image_id){
        i_check_number_of_votes_for_this(sub_id);
        List voteList = response.getBody().jsonPath().get();
        int lastVoteCount = voteList.size();
        String response_body = response.asString();
        JsonPath json_path = new JsonPath(response_body);
        List image_id_list = json_path.getList("image_id");
        String correct_image_id = image_id_list.get(lastVoteCount - 1).toString();
        System.out.println("last" + lastVoteCount + "initial" + (initialVoteCount + 1));
        System.out.println("should" + correct_image_id + "should to" + image_id);
        Assert.assertTrue(lastVoteCount == initialVoteCount + 1);
        Assert.assertEquals(correct_image_id, image_id);
        }
    }
